...
const ProjectPage = ({infoProject}) => {
  ...
  const videoRef = useRef(null)
    ...
    const { id } = useParams()
    useEffect(() => {
      const notEmptyVideo = infoProject &&  infoProject.filter(el => (el.id === parseInt(id)));
      setProjectChoose(notEmptyVideo)

      setVideoDom(
        <video ref={videoRef} controls>
        <source src={notEmptyVideo && notEmptyVideo[0].videoLink} type="video/mp4" />
  ...
  return (
    <div className='project-demo-bckg' ref={trigger} style={backgroundColor}>
        ...
        <p className="projet-name right" ref={titleTop}>{projectChoose && projectChoose[0].name}</p>
        <div className="main-content" ref = {content}>
          ...
          {projectChoose && projectChoose[0].description}
            ...
            <a className='button-link' href={projectChoose && projectChoose[0].websiteLink} target="_blank">
              <div >
                Link to the website
              </div>
            </a>
              ...
              :   <a className='button-link' href={projectChoose && projectChoose[0].repoLink} target="_blank">
                    <div >
                      Link to the repo
                    </div>
                </a>
